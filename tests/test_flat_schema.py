...
class TestFlatSchemaParse:
    ...
    def test_flat_schema_declared_keys_missing_in_doc_skipped(
        self, json_repo, tmp_path
    ):
        p = tmp_path / "b.json"
    ...
    def test_array_of_objects_still_row_per_object_with_schema(
        self, json_repo, tmp_path
    ):
...
class TestFlatSchemaSave:
    def test_save_coerces_declared_types(self, json_repo, tmp_path):
        p = tmp_path / "cfg.json"
        p.write_text(
            json.dumps({"m_Version": 9, "ServerUpdateRateLimit": 0.5, "ExtraKey": "e"}),
            encoding="utf-8",
        )
        defs, rows = json_repo.parse_file(str(p), _defs())
...
class TestXmlFlatSchema:
    def test_xml_parse_ignores_schema(self, tmp_path):
        p = tmp_path / "cfg.xml"
        p.write_text('<root><option name="a" value="1"/></root>', encoding="utf-8")
