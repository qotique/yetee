...
class TestExpansionRegistry:
    ...
    def test_trader_folder_columns(self):
        cols = get_columns(
            "Expansion Market", "mpmissions/chernarusplus/expansion/market/traders/trader.json"
        )
    ...
    def test_unknown_expansion_file_default_json(self):
        assert (
            get_renderer("Expansion Market", "market/whatever.json") == "json"
        )
...
class TestLoadProjectExpansion:
    ...
    def test_load_project_expansion_settings_uses_settings_display(self, editor, tmp_path):
        eco = tmp_path / "eco"
        settings = eco / "expansion" / "settings"
        settings.mkdir(parents=True)
        (settings / "BaseBuildingSettings.json").write_text("{}")
        ...
        assert (
            editor.current_file
            == "Mission/settings/BaseBuildingSettings.json"
        )
