    ...
    def test_nested_object_array_uses_item_schema(self):
        doc = {
            "InventoryAttachments": [
                {"SlotName": "Body", "Items": [{"ClassName": "A", "Quantity": {"Min": 0.0}}]}
            ]
        }
        schema = build_auto_form_schema(doc)
        grid = schema.grids[0]
...
class TestFormSchemaRegistry:
    def test_path_based_lookup_folder(self):
        register_form_folder_schema("X", "market/traders", FormSchema(name_key="DisplayName"))
    ...
    def test_path_based_lookup_basename(self):
        register_form_schema("X", "CoreSettings.json", FormSchema(fields=(FormField("m_Version", "Version"),)))
...
class TestFormDisplay:
    ...
    def test_load_flat_object_renders_fields(self, display, tmp_path):
        path = self._write(tmp_path, "settings.json", {"m_Version": 9, "EnableLamps": 1, "Name": "srv"})
    ...
    def test_master_files_mode(self, display, tmp_path):
        ...
        display.set_files(
            {
                "CoreSettings.json": self._write(tmp_path, "CoreSettings.json", {"m_Version": 9}),
                "GeneralSettings.json": self._write(tmp_path, "GeneralSettings.json", {"m_Version": 16}),
            }
        )
    ...
    def test_reload_within_same_entity_reuses_master_tree(self, display, tmp_path):
        ...
        display.set_files(
            {
                "CoreSettings.json": self._write(tmp_path, "CoreSettings.json", {"m_Version": 9}),
                "GeneralSettings.json": self._write(tmp_path, "GeneralSettings.json", {"m_Version": 16}),
                "SmithingSettings.json": self._write(tmp_path, "SmithingSettings.json", {"m_Version": 1}),
            }
        )
        ...
        second = next(v for k, v in display._files.items() if k != display._current_label)
        display.load_file(second)
        assert display._master.controls is controls_before
        assert display._current_label != ""
        assert display._master_rows[display._current_label].bgcolor == ft.Colors.SECONDARY_CONTAINER
    ...
    def test_master_files_grouped_by_category(self, display, tmp_path):
        ...
        tiles = [
            c
            for c in display._master.controls
            if isinstance(c, ft.ExpansionTile)
        ]
        assert [t.title.value for t in tiles] == ["Loadouts", "Settings"]
        loadout_rows = [
            row.content.value for row in tiles[0].controls if isinstance(row, ft.Container)
        ]
        assert loadout_rows == ["BanditLoadout.json"]
        settings_rows = [
            row.content.value for row in tiles[1].controls if isinstance(row, ft.Container)
        ]
    ...
    def test_master_files_subcategories(self, display, tmp_path):
        ...
        display.set_files(
            {
                "AI/FSM/fight.json": self._write(tmp_path, "fight.json", {"m_Version": 1}),
                "AI/LootDrops/zone.json": self._write(tmp_path, "zone.json", {"m_Version": 1}),
                "Quests/NPCs/bob.json": self._write(tmp_path, "bob.json", {"m_Version": 1}),
            }
        )
        display.load_file(next(iter(display._files.values())))
        assert display._master_mode == "files"
        tiles = [
            c for c in display._master.controls if isinstance(c, ft.ExpansionTile)
        ]
    ...
    def test_category_add_button_only_in_create_categories(self, display, tmp_path):
        ...
        display.set_files(
            {
                ...
                "Quests/NPCs/bob.json": self._write(tmp_path, "bob.json", {"ClassName": "bob"}),
        ...
        add_rows = [
            c for c in loadout_tile.controls if isinstance(c, ft.TextButton)
        ]
        ...
        assert [c for c in quests_sub["Objectives"].controls if isinstance(c, ft.TextButton)]
    ...
    def test_category_add_templates_from_subdir_fallback(self, display, page, tmp_path):
        ...
        display.set_entity("ExpansionMod")
        display.set_files({"Quests/Objectives/AICamp/Objective_AIC_1.json": str(sibling)})
    ...
    def test_root_array_in_files_mode_renders_editable(self, display, tmp_path):
        ...
        assert display._master_mode == "files"
        tiles = [
            c for c in display._detail.controls if isinstance(c, ft.ExpansionTile)
        ]
        assert len(tiles) == 2
    ...
    def test_category_add_creates_file(self, display, page, tmp_path):
        ...
        sibling.write_text(json.dumps([{"ClassName": "I_A", "Chance": 0.5, "Ok": True}]))
        display.set_entity("ExpansionMod")
        ...
        assert target.exists()
        assert json.loads(target.read_text()) == [{"ClassName": "", "Chance": 0, "Ok": False}]
