...
def register_form_schema(entity: str, filename: str, schema: FormSchema) -> None:
    _FORM_SCHEMAS[(entity, filename)] = schema


def register_form_folder_schema(entity: str, folder: str, schema: FormSchema) -> None:
...
def entity_has_form_schemas(entity: str, files: dict[str, str]) -> bool:
    return any(
        get_form_schema_for_path(entity, path) is not None for path in files.values()
    )
...
def _columns_from_items(items: list[dict[str, object]]) -> tuple[FormField, ...]:
    ...
    for key in keys:
        ...
        columns.append(
            FormField(key=key, label=humanize_key(key), type=_type_from_values(values))
        )
    ...
    for key, value in obj.items():
        if isinstance(value, dict):
            ...
            if value and all(isinstance(o, dict) for o in value):
                ...
                nested = any(
                    isinstance(v, (dict, list)) for o in items for v in o.values()
                )
                if nested:
                    grids.append(
                        FormGrid(
