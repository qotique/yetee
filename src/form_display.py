...
class FormDisplay:
    ...
    def _rebuild_master(self) -> None:
        ...
        elif isinstance(self._doc, list) and all(isinstance(o, dict) for o in self._doc):
    ...
    def _build_item_master(self, items: list[object]) -> None:
        ...
        for idx, item in enumerate(items):
            ...
            row = self._master_row(label, selected, on_click=self._on_item_master_click, idx=idx)
    ...
    def _on_file_master_click(self, e: ft.ControlEvent) -> None:
        ...
        if isinstance(label, str) and label != self._current_label and self.on_file_select:
    ...
    def _on_category_add(self, category: tuple[str, ...]) -> None:
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Column(
                [
                    ft.Text(
                        f"Enter a name for the new file in {'/'.join(category)}:"
                    ),
                    name_field,
                ],
                tight=True,
    ...
    def _create_category_file(self, category: tuple[str, ...], filename: str) -> None:
        ...
        try:
            self._json_repo.save_doc(
                path, self._blank_template_for(category, path)
            )
    ...
    def _field_widget(self, field: FormField, node: dict[str, object]) -> ft.Control:
        ...
        if field.type == FieldType.TOGGLE:
            cb = ft.Checkbox(
                ...
                on_change=lambda e, n=node, k=field.key: self._on_toggle(n, k, e.control.value),
            )
            return ft.Container(
                content=cb,
                expand=True,
        ...
        if field.type == FieldType.BOOL:
            cb = ft.Checkbox(
                ...
                on_change=lambda e, n=node, k=field.key: self._on_bool(n, k, e.control.value),
            )
            return ft.Container(
                content=cb,
                expand=True,
        ...
        if field.type in (FieldType.INT, FieldType.FLOAT):
            tf = ft.TextField(
                ...
                min_lines=1,
                max_lines=1,
                on_change=lambda e, n=node, k=field.key: self._on_text(n, k, e.control.value),
                expand=True,
            )
            return tf
        if field.type == FieldType.SINGLE_NAMED:
            dd = ft.Dropdown(
                ...
                on_select=lambda e, n=node, k=field.key: self._on_select(n, k, e.control.value),
                expand=True,
            )
            return dd
        tf = ft.TextField(
    ...
    def _render_field(
        self, node: dict[str, object], field: FormField
    ) -> ft.Control:
        return self._form_row(field.label, self._field_widget(field, node))

    def _render_group(
        self, node: dict[str, object], group: FormGroup
    ) -> ft.Control:
    ...
    def _render_grid(self, node: dict[str, object], grid: FormGrid) -> ft.Control:
        ...
        header = ft.Row(
            [
                ft.Container(content=ft.Text(col.label, size=11, weight=ft.FontWeight.BOLD), width=180)
    ...
    def _render_grid_nested(
        self, items: list[object], grid: FormGrid
    ) -> ft.Control:
    ...
    def _render_grid_row(
        self, grid: FormGrid, items: list[object], idx: int
    ) -> ft.Row:
        obj = items[idx] if idx < len(items) else {}
        if not isinstance(obj, dict):
            obj = {}
        ...
        for col in grid.columns:
            field = FormField(key=col.key, label=col.label, type=col.type, options=col.options)
    ...
    def _on_list_add(self, items: list[object], lst: FormList) -> None:
        default: object = "" if lst.item_type not in (FieldType.INT, FieldType.FLOAT) else 0
    ...
    def _on_dict_add(self, mapping: dict[str, object], dct: FormDict) -> None:
        default: object = "" if dct.value_type not in (FieldType.INT, FieldType.FLOAT) else 0
        suffix = 1
        key = "newKey"
    ...
    def _coerce_doc(self) -> object:
        if self._master_mode == "items" and isinstance(self._doc, list):
            items = self._doc
            return [self._coerce_object(item, self._schema) for item in items if isinstance(item, dict)]
        if isinstance(self._doc, list) and all(
            isinstance(o, dict) for o in self._doc
        ):
            return [self._coerce_object(item, self._schema) for item in self._doc if isinstance(item, dict)]
    ...
    def _coerce_object(
        ...
        for grid in schema.grids:
            ...
            if isinstance(value, list):
                out[grid.key] = [self._coerce_object(item, self._schema_for_item(grid)) for item in value if isinstance(item, dict)]
        ...
        for dct in schema.dicts:
            ...
            if isinstance(value, dict):
                out[dct.key] = {
                    str(k): _human_coerce(dct.value_type, v)
