...
class EconomyEditor:
    ...
    def load_project(self, project: Project) -> None:
        ...
        for fname, fpath in type_files.items():
            if os.path.exists(fpath):
                ...
                logger.warning("Skipping type file %s: no such file", fpath)
    ...
    def switch_entity(self, entity: str, *, force: bool = False) -> None:
        if (
            entity == self._current_entity and not force
        ) or entity not in self._entities:
            return
        ...
        if display is self._settings_display:
            ...
            self._unavailable_display is not None
            and display is self._unavailable_display
        ):
    ...
    def _add_expansion_entities(self, svc: EconomyService, economy_dir: str) -> None:
